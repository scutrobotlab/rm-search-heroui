...
export function Hit({ hit }: { hit: HitType }) {
  ...
  return (
    <Card isBlurred className="border-none h-full">
      <div className="grid grid-cols-6 md:grid-cols-12 gap-0">
        ...
        {hasImage && (
          <div className="col-span-6 md:col-span-4 mr-4 flex items-center justify-center h-full relative">
            <div className="image-gradient-mask absolute top-0 left-0 right-0 bottom-0 flex justify-center items-center">
              <Image
                isZoomed
                alt={hit.title}
                className="object-cover h-full w-full rounded-none border-none" // 去除圆角和边框
